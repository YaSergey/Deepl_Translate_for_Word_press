...
class PMT_Page_Translator
{
    ...
    public function __construct(
        ...
        callable $logger,
        callable $slug_generator,
        ?PMT_Translation_Batcher $batcher = null,
        ?PMT_ACF_Translation_Service $acf_service = null,
        ?PMT_SEO_Translation_Service $seo_service = null
    ) {
    ...
    public function translate_published_pages($target_language, $context = array())
    {
        if (!$this->can_use_polylang()) {
            return new WP_Error(
                'pmt_missing_polylang',
                __('Polylang не активирован, перевод страниц недоступен.', 'polylang-mass-translation-deepl')
            );
        }
    ...
    public function translate_single_page($page, $target_language, $context = array())
    {
        ...
        $translations         = function_exists('pll_get_post_translations')
            ? pll_get_post_translations($page->ID)
            : array();
        ...
        $translated_blocks = $this->translate_fields(
            array(
        ...
        $translation['post_name'] = call_user_func(
            $this->slug_generator,
        ...
        if (!empty($context['preview'])) {
            if (!empty($context['job_manager']) && !empty($context['job_id'])) {
                ...
                $context['job_manager']->add_entity(
                    $context['job_id'],
                    'page_preview',
                    $page->ID,
                    $preview
        ...
        if (is_wp_error($translation_id)) {
            $this->log_error(
                'Ошибка создания перевода страницы: ' . $translation_id->get_error_message()
            );
            return;
        }
        ...
        // Обновляем карту переводов для поста
        ...
        if ($this->acf_service) {
            ...
            $this->acf_service->translate_fields(
                $translation_id,
                $target_language,
                $source_language,
                $context
        ...
        if ($this->seo_service) {
            $this->seo_service->translate_seo_meta(
                $translation_id,
                $target_language,
                $source_language,
                $context
    ...
    private function translate_fields(array $fields, $source_language, $target_language)
    {
        ...
        if ($this->batcher) {
            ...
            foreach ($texts as $index => $text) {
                $result = $this->provider->translate_text(
                    $text,
                    $target_language,
                    $source_language,
                    array(
    ...
    private function copy_post_meta($from_post_id, $to_post_id, $source_language, $target_language, $context = array())
    {
        ...
        foreach ($meta as $key => $values) {
            ...
            // Синхронизируемые meta — просто копируем
            ...
            // Остальные мета-поля — кандидаты для перевода (если это строка)
            foreach ($values as $value) {
        ...
        if (!empty($translate_candidates) && $this->batcher) {
            $translated = $this->batcher->translate_batch(
        ...
        foreach ($taxonomies as $taxonomy) {
            ...
            foreach ($terms as $term) {
                $translated_term_id = function_exists('pll_get_term')
                    ? pll_get_term($term->term_id, $target_language)
                    : 0;
                ...
                if (!is_wp_error($inserted) && !empty($inserted['term_id'])) {
                    ...
                    if (function_exists('pll_save_term_translations')) {
                        $term_translations                      = function_exists('pll_get_term_translations')
                            ? pll_get_term_translations($term->term_id)
                            : array();
                        $term_translations[$target_language]    = $new_term_id;
    ...
    private function get_enabled_post_types()
    {
        ...
        // Строго ограничиваемся post и page
