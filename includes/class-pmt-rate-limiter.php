...
class PMT_Rate_Limiter
{
    ...
    /**
     * @param int           $requests_per_minute   лимит запросов в минуту
     * @param int           $characters_per_minute лимит символов в минуту
     * @param callable|null $logger                логгер
     * @param int           $characters_per_hour   лимит символов в час
     */
    public function __construct(
        $requests_per_minute = 50,
        $characters_per_minute = 120000,
        ?callable $logger = null,
        $characters_per_hour = 240000
    ) {
    ...
    /**
     * Формирует ключ для окна (минутного или часового).
     *
     * @param string $type   тип счётчика (req|char)
     * @param string $window 'minute' или 'hour'
     * @return string
     */
    ...
    private function increment($key, $amount)
    {
        ...
        $expiration = (false !== strpos($key, '_hour_'))
            ? HOUR_IN_SECONDS * 2
            : MINUTE_IN_SECONDS * 2;
    ...
    /**
     * Проверяет, проходит ли запрос по лимитам.
     *
     * @param int $char_count количество символов в запросе
     * @return bool
     */
    public function allow($char_count = 0)
    {
        $request_key   = $this->get_window_key('req', 'minute');
        $char_key      = $this->get_window_key('char', 'minute');
        ...
        $would_exceed_hour     = $this->characters_per_hour > 0
            && $chars_hour + $char_count > $this->characters_per_hour;
        ...
        if (
            $requests + 1 > max(1, floor($this->requests_per_minute * 0.8)) ||
            $chars + $char_count > max(1, floor($this->characters_per_minute * 0.8))
        ) {
