...
class PolylangMassTranslation
{
    ...
    public function translate_options_render()
    {
        ...
        echo '<label><input type="checkbox" name="' . $this->option_name . '[translate_posts]" value="1" ' . checked(1, $options['translate_posts'] ?? 1, false) . '> Переводить записи (post)</label><br>';
        echo '<label><input type="checkbox" name="' . $this->option_name . '[translate_pages]" value="1" ' . checked(1, $options['translate_pages'] ?? 1, false) . '> Переводить страницы (page)</label><br>';
    ...
    public function characters_per_hour_render()
    {
        $options = get_option($this->option_name);
        $value = isset($options['characters_per_hour']) ? (int) $options['characters_per_hour'] : 240000;
        echo '<input type="number" min="1000" name="' . $this->option_name . '[characters_per_hour]" value="' . esc_attr($value) . '" />';
        echo '<p class="description">Глобальный лимит символов в час.</p>';
    }

    public function max_chars_per_request_render()
    {
        $options = get_option($this->option_name);
        $value = isset($options['max_chars_per_request']) ? (int) $options['max_chars_per_request'] : 5000;
        echo '<input type="number" min="500" name="' . $this->option_name . '[max_chars_per_request]" value="' . esc_attr($value) . '" />';
        echo '<p class="description">Максимум символов в одном запросе к API.</p>';
    }

    public function http_timeout_render()
    {
        $options = get_option($this->option_name);
        $value = isset($options['http_timeout']) ? (int) $options['http_timeout'] : 20;
        echo '<input type="number" min="5" name="' . $this->option_name . '[http_timeout]" value="' . esc_attr($value) . '" />';
        echo '<p class="description">Таймаут HTTP-запросов к провайдерам (секунды).</p>';
    }

    public function enable_logging_render()
    {
        $options = get_option($this->option_name);
        $value = isset($options['enable_logging']) ? (int) $options['enable_logging'] : 1;
        echo '<label><input type="checkbox" name="' . $this->option_name . '[enable_logging]" value="1" ' . checked(1, $value, false) . '> Включить логирование в log.txt</label>';
    }
    ...
    public function translation_rules_render()
    {
        ...
        $rules_json_value = !empty($rules) ? wp_json_encode($rules, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) : '';
        ...
        echo '<p><label>Расширенные правила (JSON): <textarea name="' . $this->option_name . '[rules_json]" rows="4" cols="50">' . esc_textarea($rules_json_value) . '</textarea></label></p>';
    ...
    public function sanitize_settings($input)
    {
        ...
        $sanitized['translate_posts'] = isset($input['translate_posts']) ? 1 : 0;
        $sanitized['translate_pages'] = isset($input['translate_pages']) ? 1 : 0;
        ...
        $sanitized['characters_per_hour'] = max(1000, (int) ($input['characters_per_hour'] ?? 240000));
    ...
    {
        $url = admin_url('options-general.php?page=polylang-mass-translation');
        $links[] = '<a href="' . esc_url($url) . '">' . esc_html__('Settings', 'polylang-mass-translation-deepl') . '</a>';
        return $links;
    }
    ...
    public function settings_init()
    {
        ...
        $fields = array(
            array(
                'id' => 'deepl_api_key',
                'title' => 'DeepL API Key',
                'callback' => 'deepl_api_key_render',
            ),
            array(
                'id' => 'deepl_api_url',
                'title' => 'DeepL API URL',
                'callback' => 'deepl_api_url_render',
            ),
            array(
                'id' => 'translation_engine',
                'title' => __('Движок перевода', 'polylang-mass-translation-deepl'),
                'callback' => 'translation_engine_render',
            ),
            array(
                'id' => 'google_project_id',
                'title' => 'Google Cloud Project ID',
                'callback' => 'google_project_render',
            ),
            array(
                'id' => 'google_location',
                'title' => 'Google Cloud Location',
                'callback' => 'google_location_render',
            ),
            array(
                'id' => 'google_api_key',
                'title' => 'Google API Key',
                'callback' => 'google_api_key_render',
            ),
            array(
                'id' => 'google_service_account',
                'title' => __('Google Service Account JSON', 'polylang-mass-translation-deepl'),
                'callback' => 'google_service_account_render',
            ),
            array(
                'id' => 'translate_options',
                'title' => 'Что переводить',
                'callback' => 'translate_options_render',
            ),
            array(
                'id' => 'target_language',
                'title' => 'Целевой язык перевода страниц',
                'callback' => 'target_language_render',
            ),
            array(
                'id' => 'post_status',
                'title' => 'Статус созданных постов',
                'callback' => 'post_status_render',
            ),
            array(
                'id' => 'requests_per_minute',
                'title' => 'Лимит запросов в минуту',
                'callback' => 'requests_per_minute_render',
            ),
            array(
                'id' => 'characters_per_minute',
                'title' => 'Лимит символов в минуту',
                'callback' => 'characters_per_minute_render',
            ),
            array(
                'id' => 'characters_per_hour',
                'title' => __('Лимит символов в час', 'polylang-mass-translation-deepl'),
                'callback' => 'characters_per_hour_render',
            ),
            array(
                'id' => 'max_chars_per_request',
                'title' => __('Макс. символов на запрос', 'polylang-mass-translation-deepl'),
                'callback' => 'max_chars_per_request_render',
            ),
            array(
                'id' => 'http_timeout',
                'title' => __('HTTP таймаут (сек)', 'polylang-mass-translation-deepl'),
                'callback' => 'http_timeout_render',
            ),
            array(
                'id' => 'enable_logging',
                'title' => __('Вести лог', 'polylang-mass-translation-deepl'),
                'callback' => 'enable_logging_render',
            ),
            array(
                'id' => 'formality',
                'title' => 'Стиль и формальность',
                'callback' => 'formality_render',
            ),
            array(
                'id' => 'translation_rules',
                'title' => 'Правила перевода',
                'callback' => 'translation_rules_render',
            ),
        );
